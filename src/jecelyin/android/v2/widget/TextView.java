 ...
 */
@RemoteView
public class TextView extends TextViewBase implements ViewTreeObserver.OnPreDrawListener, TextViewInterface {
...
//            mInputType = inputType;
    ...
    private void setRawTextSize(float size) {
        if (size != mTextPaint.getTextSize()) {
            ...
            setLineNumberFontSize(size); //jec+
            if (mLayout != null) {
                nullLayouts();
                requestLayout();
                invalidate();
    ...
    public Parcelable onSaveInstanceState() {
        ...
        if (save) {
            ...
            ss.error = mError;
            //jec+
            ss.mUndoParcelState = mUndoParcel;
            ss.mRedoParcelState = mRedoParcel;
            //end
            return ss;
        }
    ...
    public void onRestoreInstanceState(Parcelable state) {
        ...
        //jec+
        mUndoParcel = ss.mUndoParcelState;
        mRedoParcel = ss.mRedoParcelState;
        //end
...
//        } else if (cls == EditorInfo.TYPE_CLASS_NUMBER) { //jec-
...
//            input = DialerKeyListener.getInstance();
    ...
    public void onEditorAction(int actionCode) {
        ...
        if (h != null) {
            ...
            h.sendMessage(h.obtainMessage(DISPATCH_KEY_FROM_IME,
    ...
    private void showError() {
        ...
        if (mPopup == null) {
            ...
            final android.widget.TextView err = (android.widget.TextView) inflater.inflate(R.layout.textview_hint,
                    null);
    ...
    private void invalidateCursorPath() {
        if (mHighlightPathBogus) {
            ...
            synchronized (sTempRect) {
                ...
                thick /= 2;
                thick += EditorSettings.CURSOR_WIDTH; //jec+

                mHighlightPath.computeBounds(sTempRect, false);

                //int left = getCompoundPaddingLeft(); //jec-
                int left = getCompoundPaddingLeft() + mLineNumberWidth; //jec+: 修正显示行号后光标闪烁的问题
    ...
    public boolean onPreDraw() {
        ...
        if (mCreatedWithASelection /*||
           (this instanceof ExtractEditText && selectionController != null && hasSelection()) */) { //jec: add comment
    ...
    protected void onDraw(Canvas canvas) {
            ...
            if (mCursorVisible && selStart >= 0 && isEnabled()) {
                ...
                if (selStart == selEnd) {
                    if ((SystemClock.uptimeMillis() - mShowCursor) % (2 * BLINK) < BLINK) {
                        ...
                        mHighlightPaint.setStrokeWidth(EditorSettings.CURSOR_WIDTH); //jec+: cursor width
        ...
        */
        ...
        //layout.draw(canvas, highlight, mHighlightPaint, voffsetCursor - voffsetText); //jec-
        //jec+
        layout.setLineNumberWidth(mLineNumberWidth, mLineCount);
        layout.draw(canvas
                , highlight
                , mHighlightPaint
                , voffsetCursor - voffsetText
                , mHighlight
                , EditorSettings.SHOW_LINENUM ? mLineNumberPaint : null
                , EditorSettings.SHOW_WHITESPACE ? mWhiteSpacePaint : null);
        //end
//jec-
    ...
    public boolean onKeyUp(int keyCode, KeyEvent event) {
        ...
        switch (keyCode) {
            case KeyEvent.KEYCODE_DPAD_CENTER:
                 ...
                 * this case.)
                 */
//                if (mOnClickListener == null) { //jec-
                if (!hasOnClickListener()) { //jec+
...
//        int want = width - getCompoundPaddingLeft() - getCompoundPaddingRight(); //jec-
        int want = width - getCompoundPaddingLeft() - getCompoundPaddingRight() - mLineNumberWidth; //jec+
...
//        final int x = (int)mLayout.getPrimaryHorizontal(offset); //jec-
        final int x = (int)mLayout.getPrimaryHorizontal(offset) + mLineNumberWidth; //jec+
        final int top = mLayout.getLineTop(line);
        final int bottom = mLayout.getLineTop(line + 1);

        int left = (int) FloatMath.floor(mLayout.getLineLeft(line));
//        int right = (int) FloatMath.ceil(mLayout.getLineRight(line)); //jec-
        int right = (int) FloatMath.ceil(mLayout.getLineRight(line) + mLineNumberWidth); //jec+
        int ht = mLayout.getHeight();
...
//                    awakenScrollBars(mScroller.getDuration()); //jec-: 隐藏滚动条
                    invalidate();
                } else {
...
//                if (((this instanceof ExtractEditText) || mSelectionMoved) && //jec-
                if (mSelectionMoved &&
                        selStart >= 0 && selEnd >= 0) {
                    /*
...
//            if (this instanceof ExtractEditText) { //jec-
...
//            MenuHandler handler = new MenuHandler();
            ...
            if (canCut()) {
                menu.add(0, ID_CUT, 0, android.R.string.cut).
            ...
            if (canPaste()) {
                menu.add(0, ID_PASTE, 0, android.R.string.paste).
                     setOnMenuItemClickListener(handler).
...
//            MenuHandler handler = new MenuHandler();
            ...
            if (canSelectText()) {
                if (!hasPasswordTransformationMethod()) {
                    ...
                    menu.add(0, ID_START_SELECTING_TEXT, 0, R.string.selectText).
                    setOnMenuItemClickListener(handler);
                }
                menu.add(0, ID_SELECT_ALL, 0, android.R.string.selectAll).
                     setOnMenuItemClickListener(handler).
...
//                int selEnd = getSelectionEnd();
                ...
                URLSpan[] urls = ((Spanned) mText).getSpans(min, max,
                        URLSpan.class);
                if (urls.length == 1) {
                    menu.add(0, ID_COPY_URL, 0, android.R.string.copyUrl).
                         setOnMenuItemClickListener(handler);
                    added = true;
                }
            }
...
//                     getContext().getString(android.R.string.addToDictionary, word)).
//                     setOnMenuItemClickListener(handler);
//                added = true;
//
//            }
        ...
        // 插入时间
        String date = TimeUtil.getDateByFormat(EditorSettings.DATE_FORMAT);
...
//        }
    ...
    public boolean onTextContextMenuItem(int id) {
        ...
        //end
        if (isFocused()) {
//            final int selStart = getSelectionStart(); //jec-
//            final int selEnd = getSelectionEnd();
...
//                return true;
            ...
            case R.string.convert_to_titlecase:
                ...
                if(id == R.id.to_lower)
                {
                    mText2.replace(selStart, selEnd, (new String(dest)).toLowerCase(Locale.getDefault()));
                }else if(id == R.id.to_upper)
                {
                    mText2.replace(selStart, selEnd, (new String(dest)).toUpperCase(Locale.getDefault()));
                }else{
                    String str = new String(dest);
                    mText2.replace(selStart, selEnd, str.substring(0,1).toUpperCase(Locale.getDefault())+str.substring(1));
                }
                break;
            case R.id.goto_line:
            ...
            case R.id.go_to_end:
                ...
                Selection.setSelection((Spannable) mText, len, len);
                break;
            case R.id.insert_datetime:
                String text = TimeUtil.getDateByFormat(EditorSettings.DATE_FORMAT);
                getEditableText().replace(min, max, text, 0, text.length());
                break;
            case R.id.duplicate_line:
                ...
                getEditableText().replace(offset, offset, text2, 0, text2.length());
                break;
            ...
            case R.id.doc_stat:
                ...
                String[] LineBreaks = context.getResources().getStringArray(R.array.linebreak_chars);
                sb.append(context.getString(R.string.filename))
                .append("\t\t").append(mPath).append("\n\n")
                //编码，换行符
                .append(context.getString(R.string.encoding2))
                .append("\t\t").append(mEncoding).append("\n")
                .append(context.getString(R.string.linebreak))
                .append("\t\t").append(mLinebreak < LineBreaks.length ? LineBreaks[mLinebreak] : "Unknow").append("\n")
                //总长度
    ...
    private class HandleView extends View {
        ...
        public HandleView(CursorController controller, int pos) {
            ...
            if(android.os.Build.VERSION.SDK_INT >= 11) //jec+
            mContainer.setSplitTouchEnabled(true);
            mContainer.setClippingEnabled(false);
            if(android.os.Build.VERSION.SDK_INT >= 10) //jec+
            ...
            case LEFT: {
                if (mSelectHandleLeft == null) {
                    mSelectHandleLeft = mContext.getResources().getDrawable(
                            R.drawable.text_select_handle_left); //mTextSelectHandleLeftRes); //jec
                }
            ...
            case RIGHT: {
                if (mSelectHandleRight == null) {
                    mSelectHandleRight = mContext.getResources().getDrawable(
                            R.drawable.text_select_handle_right); //mTextSelectHandleRightRes); //jec-
                }
            ...
            default: {
                if (mSelectHandleCenter == null) {
                    mSelectHandleCenter = mContext.getResources().getDrawable(
                            R.drawable.text_select_handle_middle); //mTextSelectHandleRes); //jec-
                }
        ...
        public boolean onTouchEvent(MotionEvent ev) {
            ...
            case MotionEvent.ACTION_MOVE: {
                final float rawX = ev.getRawX();
                final float rawY = ev.getRawY();
//                final float newPosX = rawX - mTouchToWindowOffsetX + mHotspotX; //jec-
                final float newPosX = rawX - mTouchToWindowOffsetX + mHotspotX - mLineNumberWidth; //jec+
...
//                    + TextView.this.mScrollX; //jec-
                + TextView.this.mScrollX + mLineNumberWidth; //jec+ 
    ...
    private static final int        VERY_WIDE = 16384*4; //jec+
...
//jec+ ----------------------------------------------------------------------------------------------------------
    private boolean hasOnClickListener()
    {
        try
        {
            Field field = getClass().getField("mOnClickListener");
            JecLog.d("hasOnClickListener = "+field);
            return true;
    ...
    public void setPath(String path)
    {
        ...
        if(fsize > EditorSettings.LIMIT_HIGHLIGHT_LENGTH)
        {
    ...
    public void updateUndoRedoButtonStatus()
    {
        if(mOnTextChangedListener != null)
            for(JecOnTextChangedListener l: mOnTextChangedListener)
                l.onTextChanged(TextView.this);
    }
    ...
    public void init()
    {
        ...
        setLineNumberFontSize(getTextSize());
        ...
        onPreferenceChanged();
        EditorSettings.addOnSharedPreferenceChanged(new OnSharedPreferenceChangeListener()
        {
    ...
    {
        if(EditorSettings.SPELLCHECK)
            setInputType(getInputType() | InputType.TYPE_TEXT_FLAG_NO_SUGGESTIONS);
        setEnabled(!EditorSettings.READONLY_MODE);
        setTypeface(EditorPreference.getFont(EditorSettings.FONT));
        setTextSize(EditorSettings.FONT_SIZE);
        // 自动换行设置
        setHorizontallyScrolling(!EditorSettings.WORD_WRAP);
        // 显示行数
        updateLineNumberWidth();
        
        setKeepScreenOn(EditorSettings.KEEP_SCREEN_ON);
        setBackgroundColor(Color.parseColor(ColorScheme.color_backgroup));
        setTextColor(Color.parseColor(ColorScheme.color_font));
    }
    ...
    {
        if(mOnTextChangedListener == null)
            mOnTextChangedListener = new ArrayList<JecOnTextChangedListener>();
        
        mOnTextChangedListener.add(l);
    }
    ...
    public void onScrollChanged(int l, int t, int oldl, int oldt)
    {
        ...
        {
            int h = mBottom - mTop - getExtendedPaddingBottom() - getExtendedPaddingTop();
    ...
    public void showIME(boolean show)
    {
        ...
        { // 显示键盘，即输入法
            ...
            if ( EditorSettings.AUTO_CAPITALIZE ){
    ...
    private Highlight mHighlight = new Highlight();
    ...
    private ArrayList<JecOnTextChangedListener> mOnTextChangedListener = null;
    ...
    private String mEncoding = "UTF-8";//当前文件的编码,用于正确回写文件
    
    private int mLinebreak = 0;//换行字符
}
